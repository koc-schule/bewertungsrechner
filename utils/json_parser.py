...
def exam_to_json(exam: Exam) -> None:
    ...
    with open(f'resources/exams/exam_{exam.exam_name}.json', 'w', encoding='utf-8') as f:
...
def json_to_exam(name: str) -> Exam:
    ...
    """
    with open(f'resources/exams/exam_{name}.json', 'r', encoding='utf-8') as f:
        data = json.load(f)
...
def course_to_json(course: Course) -> None:
    ...
    with open(f'resources/courses/course_{course.course_name}.json', 'w', encoding='utf-8') as f:
        json.dump(data, f, ensure_ascii=False, indent=4)
...
def json_to_course(name: str) -> Course:
    ...
    """
    with open(f'resources/courses/course_{name}.json', 'r', encoding='utf-8') as f:
        data = json.load(f)
